...
TEST_TORTOISECOIN_BINARY="src/test/test_tortoisecoin"
...
if [[ ! -e ${TEST_TORTOISECOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_TORTOISECOIN_BINARY} not found. Run \"cmake -B build -DCMAKE_BUILD_TYPE=Coverage\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_TORTOISECOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
