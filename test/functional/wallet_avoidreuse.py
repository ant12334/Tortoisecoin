...
class AvoidReuseTest(TortoisecoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 TTC with
        the avoid_reuse flag set to false. This means the 10 TTC send should succeed,
        ...
        # listunspent should show 1 single, unused 10 TTC output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 TTC trusted
        ...
        # listunspent should show 1 single, unused 5 TTC output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 TTC trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        self.nodes[0].sendtoaddress(fundaddr, 10)
        self.generate(self.nodes[0], 1)

        # listunspent should show 2 total outputs (5, 10 TTC), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 TTC trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 TTC), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 TTC trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 TTC left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 TTC to A.
        [1] spends 5 TTC from A. (leaving roughly 5 TTC useable)
        [0] sends 10 TTC to A again.
        [1] tries to spend 10 TTC (fails; dirty).
        [1] tries to spend 4 TTC (succeeds; change address sufficient)
        '''
        ...
        if not self.options.descriptors:
            ...
            # listunspent should show 2 total outputs (1, 10 TTC), one unused (1), one reused (10)
            assert_unspent(self.nodes[1], total_count=2, total_sum=11, reused_count=1, reused_sum=10)
            # getbalances should show 10 used, 1 TTC trusted
            assert_balances(self.nodes[1], mine={"used": 10, "trusted": 1})

            # node 1 should now have about 1 TTC left (no dirty) and 11 (including dirty)
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 101 outputs of 1 TTC in the same reused
        address and tries to send a small payment of 0.5 TTC. The wallet
        should use 100 outputs from the reused address as inputs and not a
        single 1 TTC input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 101 outputs of 1 TTC to the same, reused address in the wallet
        for _ in range(101):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 202 outputs of 1 TTC in the same reused
        address and tries to send a payment of 200.5 TTC. The wallet
        ...
        # Send 202 outputs of 1 TTC to the same, reused address in the wallet
        for _ in range(202):
            self.nodes[0].sendtoaddress(new_addr, 1)
