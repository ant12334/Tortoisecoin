...
class RPCPackagesTest(TortoisecoinTestFramework):
    ...
    def test_maxfeerate_submitpackage(self):
        ...
        minrate_TTC_kvb = min([chained_txn["fee"] / chained_txn["tx"].get_vsize() * 1000 for chained_txn in chained_txns])
        chain_hex = [t["hex"] for t in chained_txns]
        pkg_result = node.submitpackage(chain_hex, maxfeerate=minrate_TTC_kvb - Decimal("0.00000001"))
    ...
    def test_maxburn_submitpackage(self):
        ...
        minrate_TTC_kvb_burn = min([chained_txn_burn["fee"] / chained_txn_burn["tx"].get_vsize() * 1000 for chained_txn_burn in chained_txns_burn])

        # Relax the restrictions for both and send it; parent gets through as own subpackage
        pkg_result = node.submitpackage(chained_burn_hex, maxfeerate=minrate_TTC_kvb_burn, maxburnamount=chained_txns_burn[1]["new_utxo"]["value"])
