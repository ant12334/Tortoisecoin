...
"""Check that it's not possible to start a second tortoisecoind instance using the same datadir or wallet."""
import random
import string

from test_framework.test_framework import TortoisecoinTestFramework
from test_framework.test_node import (
    TORTOISECOIN_PID_FILENAME_DEFAULT,
    ErrorMatch,
)

class FilelockTest(TortoisecoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second tortoisecoind instance using the same datadir")
        ...
        self.log.info("Check that we can't start a second tortoisecoind instance using the same blocksdir")
        ...
        self.log.info("Check that cookie and PID file are not deleted when attempting to start a second tortoisecoind using the same datadir/blocksdir")
        cookie_file = datadir / ".cookie"
        assert cookie_file.exists()  # should not be deleted during the second tortoisecoind instance shutdown
        pid_file = datadir / TORTOISECOIN_PID_FILENAME_DEFAULT
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second tortoisecoind instance using the same wallet")
                if descriptors:
