...
class WalletTest(TortoisecoinTestFramework):
    ...
    def run_test(self):
        ...
        # Sendmany 5 TTC to two addresses with subtracting fee from both addresses
        ...
        self.log.info("Test sendmany with fee_rate param (explicit fee rate in min/vB)")
        fee_rate_sat_vb = 2
        fee_rate_TTC_kvb = fee_rate_sat_vb * 1e3 / 1e8
        explicit_fee_rate_TTC_kvb = Decimal(fee_rate_TTC_kvb) / 1000

        # Test passing fee_rate as a string
        txid = self.nodes[2].sendmany(amounts={address: 10}, fee_rate=str(fee_rate_sat_vb))
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - Decimal('10'), explicit_fee_rate_TTC_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        ...
        self.generate(self.nodes[2], 1, sync_fun=lambda: self.sync_all(self.nodes[0:3]))
        balance = self.nodes[2].getbalance()
        node_2_bal = self.check_fee_amount(balance, node_2_bal - amount, explicit_fee_rate_TTC_kvb, self.get_vsize(self.nodes[2].gettransaction(txid)['hex']))
        assert_equal(balance, node_2_bal)
        node_0_bal += amount
        assert_equal(self.nodes[0].getbalance(), node_0_bal)
        ...
        assert_raises_rpc_error(-6, "Fee rate (0.999 min/vB) is lower than the minimum fee rate setting (1.000 min/vB)",
        ...
        msg = "Fee rate (0.000 min/vB) is lower than the minimum fee rate setting (1.000 min/vB)"
        ...
        for target, mode in product([-1, 0], ["TTC/kb", "min/b"]):
        ...
        if not self.options.descriptors:
            ...
            self.log.info("Test sendtoaddress with fee_rate param (explicit fee rate in min/vB)")
            ...
            assert_fee_amount(fee, tx_size, Decimal(fee_rate_TTC_kvb))
