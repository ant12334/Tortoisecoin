...
class BackwardsCompatibilityTest(TortoisecoinTestFramework):
    ...
    def test_v19_addmultisigaddress(self):
        ...
        # Copy the 0.19 wallet to the last Tortoisecoin Core version and open it:
        shutil.copytree(
    ...
    def run_test(self):
        ...
        if self.options.descriptors:
            ...
            for node in legacy_only_nodes:
                # RPC loadwallet failure causes tortoisecoind to exit in <= 0.17, in addition to the RPC
        ...
        if self.options.descriptors:
            ...
            node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Tortoisecoin Core")
        ...
        for node in descriptors_nodes if self.options.descriptors else legacy_nodes:
            ...
            # Make sure this wallet opens with only the migration warning. See https://github.com/tortoisecoin/tortoisecoin/pull/19054
