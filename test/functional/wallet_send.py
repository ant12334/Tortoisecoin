...
class WalletSendTest(TortoisecoinTestFramework):
    ...
    def set_test_params(self):
        ...
        getcontext().prec = 8 # mini precision for Decimal
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Use fee_rate (min/vB) instead of feeRate", w0.send, {w1.getnewaddress(): 1}, 6, "conservative", 1, {"feeRate": 0.01})
        ...
        # Test setting explicit fee rate just below the minimum of 1 min/vB.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' if fee_rate of 0.99999999 is passed")
        msg = "Fee rate (0.999 min/vB) is lower than the minimum fee rate setting (1.000 min/vB)"
        ...
        # res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0.1, estimate_mode="min/b", add_to_wallet=False)
        # assert res["hex"]
        # hex = res["hex"]
        ...
        self.test_send(from_wallet=w0, to_wallet=w1, amount=1, change_address="not an address",
                       expect_error=(-5, "Change address must be a valid tortoisecoin address"))
