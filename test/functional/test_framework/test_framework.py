...
class TortoisecoinTestMetaClass(type):
    """Metaclass for TortoisecoinTestFramework.

    Ensures that any attempt to register a subclass of `TortoisecoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'TortoisecoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("TortoisecoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("TortoisecoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class TortoisecoinTestFramework(metaclass=TortoisecoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "tortoisecoind": ("tortoisecoind", "TORTOISECOIND"),
            "tortoisecoin-cli": ("tortoisecoincli", "TORTOISECOINCLI"),
            "tortoisecoin-util": ("tortoisecoinutil", "TORTOISECOINUTIL"),
            "tortoisecoin-wallet": ("tortoisecoinwallet", "TORTOISECOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("TortoisecoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'tortoisecoind', self.options.tortoisecoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'tortoisecoin-cli', self.options.tortoisecoincli) for v in versions]
        ...
        for i in range(num_nodes):
            ...
            test_node_i = TestNode(
                ...
                tortoisecoind=binary[i],
                tortoisecoin_cli=binary_cli[i],
                version=versions[i],
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    tortoisecoind=self.options.tortoisecoind,
                    tortoisecoin_cli=self.options.tortoisecoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
    ...
    def is_tortoisecoin_util_compiled(self):
        ...
        return self.config["components"].getboolean("ENABLE_TORTOISECOIN_UTIL")
