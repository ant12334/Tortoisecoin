...
def assert_fee_amount(fee, tx_size, feerate_TTC_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_TTC_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s TTC too low! (Should be %s TTC)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_TTC_kvB)
    if fee > high_fee:
...
def get_fee(tx_size, feerate_TTC_kvb):
    """Calculate the fee in TTC given a feerate is TTC/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_TTC_kvb * Decimal(1e8)) # Fee in min/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest min
    return target_fee_sat / Decimal(1e8) # Return result in  TTC
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `TortoisecoinTestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "tortoisecoin.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Tortoisecoin"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Tortoisecoin"
        else:
            datadir = temp_dir / ".tortoisecoin"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "tortoisecoin.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "tortoisecoin.conf")):
        with open(os.path.join(datadir, "tortoisecoin.conf"), 'r', encoding='utf8') as f:
            for line in f:
