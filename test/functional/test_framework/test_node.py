...
BITCOIN_PID_FILENAME_DEFAULT = "tortoisecoind.pid"
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            try:
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; tortoisecoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                suppressed_errors["missing_credentials"] += 1
                latest_error = repr(e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to tortoisecoind after {self.rpc_timeout}s (ignored errors: {str(dict(suppressed_errors))}, latest error: {latest_error})")
