...
"""Class for tortoisecoind node under test"""
...
TORTOISECOIND_PROC_WAIT_TIMEOUT = 60
# The size of the blocks xor key
# from InitBlocksdirXorKey::xor_key.size()
NUM_XOR_BYTES = 8
# The null blocks key (all 0s)
NULL_BLK_XOR_KEY = bytes([0] * NUM_XOR_BYTES)
TORTOISECOIN_PID_FILENAME_DEFAULT = "tortoisecoind.pid"
...
class TestNode():
    """A class for representing a tortoisecoind node under test.
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, tortoisecoind, tortoisecoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False, v2transport=False):
        """
        Kwargs:
        ...
        # This means that starting a tortoisecoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(tortoisecoin_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any tortoisecoind processes lying around after
        # the test ends
        if self.process:
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # If listening and no -bind is given, then tortoisecoind would bind P2P ports on
        ...
        # Add a new stdout and stderr file each time tortoisecoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by tortoisecoind, and
        ...
        self.running = True
        self.log.debug("tortoisecoind started, waiting for RPC to come up")

        if self.start_perf:
            self._start_perf()

    def wait_for_rpc_connection(self, *, wait_for_import=True):
        """Sets up an RPC connection to the tortoisecoind process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'tortoisecoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; tortoisecoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to tortoisecoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; tortoisecoind is still starting
    ...
    def wait_until_stopped(self, *, timeout=TORTOISECOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def replace_in_config(self, replacements):
        ...
        The substitutions are passed as a list of search-replace-tuples, e.g.
            [("old", "new"), ("foo", "bar"), ...]
        """
        with open(self.tortoisecoinconf, 'r', encoding='utf8') as conf:
            conf_data = conf.read()
        for replacement in replacements:
            assert_equal(len(replacement), 2)
            old, new = replacement[0], replacement[1]
            conf_data = conf_data.replace(old, new)
        with open(self.tortoisecoinconf, 'w', encoding='utf8') as conf:
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into tortoisecoind")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to tortoisecoind
        expected_msg: regex that stderr should match when tortoisecoind fails

        Will throw if tortoisecoind starts without an error.
        Will throw if an expected_msg is provided and it does not match tortoisecoind's stdout."""
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'tortoisecoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'tortoisecoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir):
        ...
        self.input = None
        self.log = logging.getLogger('TestFramework.tortoisecoincli')
