...
class RawTransactionsTest(TortoisecoinTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid tortoisecoin address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate min/vB and feeRate TTC/kvB)")
        node = self.nodes[3]
        ...
        TTC_kvb_to_sat_vb = 100000  # (1e5)
        result1 = node.fundrawtransaction(rawtx, fee_rate=str(2 * TTC_kvb_to_sat_vb * self.min_relay_tx_fee))
        result2 = node.fundrawtransaction(rawtx, feeRate=2 * self.min_relay_tx_fee)
        result3 = node.fundrawtransaction(rawtx, fee_rate=10 * TTC_kvb_to_sat_vb * self.min_relay_tx_fee)
        ...
        self.log.info("Test min fee rate checks are bypassed with fundrawtxn, e.g. a fee_rate under 1 min/vB is allowed")
        node.fundrawtransaction(rawtx, fee_rate=0.999, add_inputs=True)
        node.fundrawtransaction(rawtx, feeRate=0.00000999, add_inputs=True)

        self.log.info("- raises RPC error if both feeRate and fee_rate are passed")
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (min/vB) and feeRate (TTC/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (TTC/kvB)
        ...
        # Test subtract fee from outputs with fee_rate (min/vB)
        TTC_kvb_to_sat_vb = 100000  # (1e5)
        result = [self.nodes[3].fundrawtransaction(rawtx),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[]),  # empty subtraction list
            self.nodes[3].fundrawtransaction(rawtx, subtractFeeFromOutputs=[0]),  # uses self.min_relay_tx_fee (set by settxfee)
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * TTC_kvb_to_sat_vb * self.min_relay_tx_fee),
            self.nodes[3].fundrawtransaction(rawtx, fee_rate=2 * TTC_kvb_to_sat_vb * self.min_relay_tx_fee, subtractFeeFromOutputs=[0]),]
        dec_tx = [self.nodes[3].decoderawtransaction(tx_['hex']) for tx_ in result]
        output = [d['vout'][1 - r['changepos']]['value'] for d, r in zip(dec_tx, result)]
        change = [d['vout'][r['changepos']]['value'] for d, r in zip(dec_tx, result)]
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 TTC outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 TTC
        ...
        # Fund wallet with 2 outputs, 5 TTC each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 TTC each (20 TTC total)
        ...
        # Attempt to send 29 TTC from a wallet that only has 20 TTC. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 TTC payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 TTC.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 TTC)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 TTC.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 TTC (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 TTC/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # At a feerate of 1.85 min/vb, the input will need a fee of 125.8 sats and the rest 77.7 sats
        ...
        # fail with insufficient funds rather than tortoisecoind asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 TTC target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 TTC) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 TTC covered (need to fund 0.9 TTC more).
