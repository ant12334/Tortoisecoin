...
random value payments which sum to 10TTC for each output type to Bob,
for a total of 40TTC in Bob's wallet.
...
class AddressInputTypeGrouping(BitcoinTestFramework):
    ...
    def make_payment(self, A, B, v, addr_type):
        ...
        self.log.debug(f"Making payment of {v} TTC at fee_rate {fee_rate}")
        tx = B.sendtoaddress(
    ...
    def run_test(self):
        ...
        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} TTC to legacy")
            A.sendtoaddress(B.getnewaddress(address_type="legacy"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} TTC to p2sh")
            A.sendtoaddress(B.getnewaddress(address_type="p2sh-segwit"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} TTC to bech32")
            A.sendtoaddress(B.getnewaddress(address_type="bech32"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} TTC to bech32m")
