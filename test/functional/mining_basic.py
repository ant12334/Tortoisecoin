...
DEFAULT_BLOCK_MIN_TX_FEE = 1000  # default `-blockmintxfee` setting [min/kvB]
...
class MiningTest(TortoisecoinTestFramework):
    ...
    def test_blockmintxfee_parameter(self):
        ...
        # test default (no parameter), zero and a bunch of arbitrary blockmintxfee rates [min/kvB]
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            blockmintxfee_TTC_kvb = blockmintxfee_sat_kvb / Decimal(COIN)
            if blockmintxfee_sat_kvb == DEFAULT_BLOCK_MIN_TX_FEE:
                self.log.info(f"-> Default -blockmintxfee setting ({blockmintxfee_sat_kvb} min/kvB)...")
            else:
                blockmintxfee_parameter = f"-blockmintxfee={blockmintxfee_TTC_kvb:.8f}"
                self.log.info(f"-> Test {blockmintxfee_parameter} ({blockmintxfee_sat_kvb} min/kvB)...")
            ...
            tx_with_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_TTC_kvb)
            assert_equal(tx_with_min_feerate["fee"], get_fee(tx_with_min_feerate["tx"].get_vsize(), blockmintxfee_TTC_kvb))
            if blockmintxfee_TTC_kvb > 0:
                lowerfee_TTC_kvb = blockmintxfee_TTC_kvb - Decimal(10)/COIN  # 0.01 min/vbyte lower
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=lowerfee_TTC_kvb)
                assert_equal(tx_below_min_feerate["fee"], get_fee(tx_below_min_feerate["tx"].get_vsize(), lowerfee_TTC_kvb))
            else:  # go below zero fee by using modified fees
                tx_below_min_feerate = self.wallet.send_self_transfer(from_node=node, fee_rate=blockmintxfee_TTC_kvb)
