...
DUST_RELAY_TX_FEE = 3000  # default setting [min/kvB]
...
class DustRelayFeeTest(TortoisecoinTestFramework):
    ...
    def run_test(self):
        ...
        # test default (no parameter), disabled (=0) and a bunch of arbitrary dust fee rates [min/kvB]
        for dustfee_sat_kvb in (DUST_RELAY_TX_FEE, 0, 1, 66, 500, 1337, 12345, 21212, 333333):
            dustfee_TTC_kvb = dustfee_sat_kvb / Decimal(COIN)
            if dustfee_sat_kvb == DUST_RELAY_TX_FEE:
                self.log.info(f"Test default dust limit setting ({dustfee_sat_kvb} min/kvB)...")
            else:
                dust_parameter = f"-dustrelayfee={dustfee_TTC_kvb:.8f}"
                self.log.info(f"Test dust limit setting {dust_parameter} ({dustfee_sat_kvb} min/kvB)...")
                self.restart_node(0, extra_args=[dust_parameter, "-permitbaremultisig"])

            for output_script, description in output_scripts:
                self.test_dust_output(self.nodes[0], dustfee_TTC_kvb, output_script, description)
