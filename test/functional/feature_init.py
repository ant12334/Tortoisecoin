...
class InitTest(TortoisecoinTestFramework):
    """
    ...
    def init_stress_test(self):
        ...
        def sigterm_node():
            if platform.system() == 'Windows':
                ...
                # tortoisecoind to perform any shutdown logic.
    ...
    def init_pid_test(self):
        TORTOISECOIN_PID_FILENAME_CUSTOM = "my_fancy_tortoisecoin_pid_file.foobar"

        self.log.info("Test specifying custom pid file via -pid command line option")
        custom_pidfile_relative = TORTOISECOIN_PID_FILENAME_CUSTOM
        ...
        assert not (datadir / TORTOISECOIN_PID_FILENAME_DEFAULT).exists()
        assert (datadir / custom_pidfile_relative).exists()
        self.stop_node(0)
        assert not (datadir / custom_pidfile_relative).exists()

        custom_pidfile_absolute = Path(self.options.tmpdir) / TORTOISECOIN_PID_FILENAME_CUSTOM
