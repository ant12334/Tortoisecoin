...
class MempoolLimitTest(TortoisecoinTestFramework):
    ...
    def test_mid_package_eviction_success(self):
        ...
        fees_TTC_per_kvb = [entry["fees"]["base"] / (Decimal(entry["vsize"]) / 1000) for entry in mempool_entries]
        mempool_entry_minrate = min(fees_TTC_per_kvb)
        ...
        for eviction in eviction_set:
            ...
            for txid, entry in zip(mempool_txids, mempool_entries):
                if txid == eviction:
                    evicted_feerate_TTC_per_kvb = entry["fees"]["modified"] / (Decimal(entry["vsize"]) / 1000)
                    assert_greater_than(evicted_feerate_TTC_per_kvb, max_parent_feerate)
    ...
    def run_test(self):
        ...
        worst_feerate_TTCvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_TTCvb = min(worst_feerate_TTCvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_feerate = (worst_feerate_TTCvb * 1000) - Decimal("0.000001")  # 0.1 sats/vbyte below worst feerate
        ...
        assert_greater_than(worst_feerate_TTCvb, package_fee / package_vsize)
