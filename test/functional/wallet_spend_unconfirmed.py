...
class UnconfirmedInputTest(TortoisecoinTestFramework):
    ...
    def test_confirmed_and_unconfirmed_parent(self):
        ...
        self.generate(self.nodes[0], 1) # Wallet has two confirmed UTXOs of ~1TTC each
        unconfirmed_parent_txid = wallet.sendtoaddress(address=wallet.getnewaddress(), amount=0.5, fee_rate=0.5*self.target_fee_rate)

        # wallet has one confirmed UTXO of 1TTC and two unconfirmed UTXOs of ~0.5TTC each
