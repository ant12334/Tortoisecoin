...
to a hash that has been compiled into tortoisecoind.
...
class AssumeutxoTest(TortoisecoinTestFramework):
    ...
    def test_invalid_mempool_state(self, dump_output_path):
        self.log.info("Test tortoisecoind should fail when mempool not empty.")
        node=self.nodes[2]
    ...
    def test_invalid_file_path(self):
        self.log.info("Test tortoisecoind should fail when file path is invalid.")
        node = self.nodes[0]
    ...
    def test_snapshot_with_less_work(self, dump_output_path):
        self.log.info("Test tortoisecoind should fail when snapshot has less accumulated work than this node.")
        node = self.nodes[0]
