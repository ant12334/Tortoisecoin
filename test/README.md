This directory contains integration tests that test tortoisecoind and its
...
tortoisecoind and tortoisecoin-qt by interacting with them through the RPC and P2P
interfaces.
...
The P2P and RPC ports used by the tortoisecoind nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another tortoisecoind
process running on the system (perhaps from a previous test which hasn't successfully
killed all its tortoisecoind nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other tortoisecoind processes are running.

On linux, the test framework will warn if there is another
tortoisecoind process running when the tests are started.

If there are zombie tortoisecoind processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
tortoisecoind processes running on the system, so should not be used if any non-test
tortoisecoind processes are being run.**

```bash
killall tortoisecoind
```

or

```bash
pkill -9 tortoisecoind
```
...
sure tortoisecoind processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and tortoisecoind `debug.log`s will all be dumped
...
`test_framework.log` and tortoisecoind `debug.log`s can be combined into a single
...
call methods that interact with the tortoisecoind nodes-under-test.

If further introspection of the tortoisecoind instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/tortoisecoind.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/tortoisecoind <pid>
```
