...
const std::string TortoisecoinGUI::DEFAULT_UIPLATFORM =
...
TortoisecoinGUI::TortoisecoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &TortoisecoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &TortoisecoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &TortoisecoinGUI::showModalOverlay);
...
TortoisecoinGUI::~TortoisecoinGUI()
{
...
void TortoisecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Tortoisecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tortoisecoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &TortoisecoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &TortoisecoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &TortoisecoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Tortoisecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Tortoisecoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Tortoisecoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Tortoisecoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a tortoisecoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Tortoisecoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &TortoisecoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &TortoisecoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &TortoisecoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &TortoisecoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &TortoisecoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &TortoisecoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &TortoisecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &TortoisecoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &TortoisecoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &TortoisecoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &TortoisecoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &TortoisecoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &TortoisecoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &TortoisecoinGUI::showDebugWindow);
}

void TortoisecoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void TortoisecoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &TortoisecoinGUI::setCurrentWalletBySelectorIndex);
...
void TortoisecoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &TortoisecoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &TortoisecoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &TortoisecoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &TortoisecoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void TortoisecoinGUI::enableHistoryAction(bool privacy)
{
...
void TortoisecoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &TortoisecoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &TortoisecoinGUI::removeWallet);
...
WalletController* TortoisecoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void TortoisecoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &TortoisecoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &TortoisecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &TortoisecoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &TortoisecoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &TortoisecoinGUI::incomingTransaction);
    connect(this, &TortoisecoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void TortoisecoinGUI::removeWallet(WalletModel* walletModel)
{
...
void TortoisecoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void TortoisecoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void TortoisecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void TortoisecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void TortoisecoinGUI::createTrayIcon()
{
...
void TortoisecoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &TortoisecoinGUI::toggleHidden);
...
void TortoisecoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void TortoisecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void TortoisecoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void TortoisecoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void TortoisecoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void TortoisecoinGUI::openClicked()
{
...
void TortoisecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void TortoisecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void TortoisecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void TortoisecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void TortoisecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void TortoisecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void TortoisecoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void TortoisecoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Tortoisecoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void TortoisecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void TortoisecoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void TortoisecoinGUI::updateHeadersSyncProgressLabel()
{
...
void TortoisecoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void TortoisecoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &TortoisecoinGUI::quitRequested);
...
void TortoisecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void TortoisecoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
#ifndef USE_SQLITE
    ...
    connect(activity, &CreateWalletActivity::created, this, &TortoisecoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void TortoisecoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void TortoisecoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &TortoisecoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &TortoisecoinGUI::show);
                e->ignore();
            }
        }
    }
...
void TortoisecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void TortoisecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void TortoisecoinGUI::incomingTransaction(const QString& date, TortoisecoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(TortoisecoinUnits::formatWithUnit(unit, amount, true));
...
void TortoisecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void TortoisecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool TortoisecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool TortoisecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void TortoisecoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void TortoisecoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void TortoisecoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void TortoisecoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void TortoisecoinGUI::updateWindowTitle()
{
...
void TortoisecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void TortoisecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void TortoisecoinGUI::detectShutdown()
{
...
void TortoisecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void TortoisecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(TortoisecoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = TortoisecoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void TortoisecoinGUI::subscribeToCoreSignals()
{
...
void TortoisecoinGUI::unsubscribeFromCoreSignals()
{
...
bool TortoisecoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<TortoisecoinUnit> units = TortoisecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const TortoisecoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, TortoisecoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const TortoisecoinUnit u : TortoisecoinUnits::availableUnits()) {
        menu->addAction(TortoisecoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(TortoisecoinUnit newUnits)
{
    setText(TortoisecoinUnits::longName(newUnits));
}
