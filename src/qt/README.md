This directory contains the source code for the Tortoisecoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### tortoisecoingui.(h/cpp)

- Represents the main window of the Tortoisecoin UI.
...
- ClientModel is used by the main application `tortoisecoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a tortoisecoin:... link from a browser).
...
* UI elements like TortoisecoinAmountField, which inherit from QWidget.
...
**Note:** Do not change `local/tortoisecoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `tortoisecoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `tortoisecoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `tortoisecoin-qt`
9. You're all set! Start developing, building, and debugging the Tortoisecoin Core GUI
