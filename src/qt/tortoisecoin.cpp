...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<TortoisecoinUnit>("TortoisecoinUnit");
#else
    qRegisterMetaType<TortoisecoinUnit>("TortoisecoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. tortoisecoin_de.qm (shortcut "de" needs to be defined in tortoisecoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. tortoisecoin_de_DE.qm (shortcut "de_DE" needs to be defined in tortoisecoin.qrc)
...
static const char* qt_argv = "tortoisecoin-qt";

TortoisecoinApplication::TortoisecoinApplication()
...
void TortoisecoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the TortoisecoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", TortoisecoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void TortoisecoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool TortoisecoinApplication::createOptionsModel(bool resetSettings)
{
...
void TortoisecoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new TortoisecoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &TortoisecoinGUI::quitRequested, this, &TortoisecoinApplication::requestShutdown);
...
void TortoisecoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void TortoisecoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool TortoisecoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void TortoisecoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &TortoisecoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &TortoisecoinApplication::handleRunawayException);
    connect(this, &TortoisecoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &TortoisecoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void TortoisecoinApplication::parameterSetup()
{
...
void TortoisecoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void TortoisecoinApplication::requestInitialize()
{
...
void TortoisecoinApplication::requestShutdown()
{
...
void TortoisecoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // tortoisecoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TortoisecoinGUI::handlePaymentRequest);
            connect(window, &TortoisecoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void TortoisecoinApplication::handleRunawayException(const QString &message)
{
...
void TortoisecoinApplication::handleNonFatalException(const QString& message)
{
...
WId TortoisecoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool TortoisecoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", TortoisecoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(tortoisecoin);
    Q_INIT_RESOURCE(tortoisecoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(TORTOISECOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tortoisecoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see tortoisecoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse tortoisecoin.conf, determine network, switch to network specific
    ...
    // tortoisecoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
