...
#include <qt/tortoisecoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Tortoisecoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new TortoisecoinAddressEntryValidator(parent));
    widget->setCheckValidator(new TortoisecoinAddressCheckValidator(parent));
}
...
bool parseTortoisecoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no tortoisecoin: URI
    if(!uri.isValid() || uri.scheme() != QString("tortoisecoin"))
        return false;
...
bool parseTortoisecoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseTortoisecoinURI(uriInstance, out);
}

QString formatTortoisecoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("tortoisecoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openTortoisecoinConf()
{
    ...
    /* Open tortoisecoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tortoisecoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Tortoisecoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Tortoisecoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Tortoisecoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "tortoisecoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("tortoisecoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a tortoisecoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Tortoisecoin\n";
        else
            optionFile << strprintf("Name=Tortoisecoin (%s)\n", ChainTypeToString(chain));
