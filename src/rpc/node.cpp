    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new tortoisecoin-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new tortoisecoin-echo executable,
                // and spawn tortoisecoin-echo below instead of tortoisecoin-node. But
                // using tortoisecoin-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("tortoisecoin-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a tortoisecoind
                // process not a tortoisecoind-node process, so just create a local
