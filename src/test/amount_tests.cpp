...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.00000002 TTC/kvB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.00000001 TTC/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::TTC_KVB), "0.00000001 TTC/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::SAT_VB), "0.001 min/vB");
}
