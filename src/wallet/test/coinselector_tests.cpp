...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 TTC in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 TTC in 1 coin
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 TTC total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 TTC total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(5000), 144, false, 0, true);
            }
            return available_coins;
        });
...

    {
        ...
        const auto& res = CoinGrinder(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            // Expected Result: 4 + 3 + 2 + 1 = 10 TTC at 400 vB
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_selection_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 10; ++j) {
                /* 10 × 1 TTC + 10 × 2 TTC = 30 TTC. 20 × 272 WU = 5440 WU */
...
BOOST_AUTO_TEST_CASE(check_max_selection_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 TTC and 1515x 0.033 TTC (~100.0 TTC total) unspent outputs
        // Then tries to spend 49.5 TTC
        // The 50.0 TTC output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that the 50 TTC UTXO was selected, and result is below max_weight
...

    {
        ...
        // The actor starts with 400x 0.0625 TTC and 2000x 0.025 TTC (75.0 TTC total) unspent outputs
        // Then tries to spend 49.5 TTC
...

    {
        ...
        // The actor starts with 1515x 0.033 TTC (49.995 TTC total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 TTC
    }

    CAmount target{99900}; // 0.000999 TTC
