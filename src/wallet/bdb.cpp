...
//! so tortoisecoin should never create different databases with the same fileid, but
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool tortoisecoin-wallet to salvage or restoring a backup."), fs::quoted(fs::PathToString(file_path)));
            return false;
        }
    }
