...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see tortoisecoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (tortoisecoind) is a headless program that connects to the Tortoisecoin network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for tortoisecoind but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect tortoisecoind signal handlers
    noui_connect();
