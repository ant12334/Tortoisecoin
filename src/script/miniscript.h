...
#ifndef TORTOISECOIN_SCRIPT_MINISCRIPT_H
#define TORTOISECOIN_SCRIPT_MINISCRIPT_H
...
struct InputResult {
    ...
    InputResult(A&& in_nsat, B&& in_sat) : nsat(std::forward<A>(in_nsat)), min(std::forward<B>(in_sat)) {}
};
...
struct Ops {
    ...
    Ops(uint32_t in_count, MaxInt<uint32_t> in_sat, MaxInt<uint32_t> in_dsat) : count(in_count), min(in_sat), dsat(in_dsat) {};
};
...
struct StackSize {
    const SatInfo min, dsat;

    constexpr StackSize(SatInfo in_sat, SatInfo in_dsat) noexcept : min(in_sat), dsat(in_dsat) {};
    constexpr StackSize(SatInfo in_both) noexcept : min(in_both), dsat(in_both) {};
};
...
struct WitnessSize {
    ...
    WitnessSize(MaxInt<uint32_t> in_sat, MaxInt<uint32_t> in_dsat) : min(in_sat), dsat(in_dsat) {};
};
...
private:
    internal::Ops CalcOps() const {
        switch (fragment) {
            ...
            case Fragment::AND_V: return {subs[0]->ops.count + subs[1]->ops.count, subs[0]->ops.min + subs[1]->ops.min, {}};
            case Fragment::AND_B: {
                const auto count{1 + subs[0]->ops.count + subs[1]->ops.count};
                const auto min{subs[0]->ops.min + subs[1]->ops.min};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, min, dsat};
            }
            case Fragment::OR_B: {
                const auto count{1 + subs[0]->ops.count + subs[1]->ops.count};
                const auto min{(subs[0]->ops.min + subs[1]->ops.dsat) | (subs[1]->ops.min + subs[0]->ops.dsat)};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, min, dsat};
            }
            case Fragment::OR_D: {
                const auto count{3 + subs[0]->ops.count + subs[1]->ops.count};
                const auto min{subs[0]->ops.min | (subs[1]->ops.min + subs[0]->ops.dsat)};
                const auto dsat{subs[0]->ops.dsat + subs[1]->ops.dsat};
                return {count, min, dsat};
            }
            case Fragment::OR_C: {
            ...
            case Fragment::ANDOR: {
                ...
                const auto min{(subs[1]->ops.min + subs[0]->ops.min) | (subs[0]->ops.dsat + subs[2]->ops.min)};
            ...
            case Fragment::WRAP_C:
            case Fragment::WRAP_N: return {1 + subs[0]->ops.count, subs[0]->ops.min, subs[0]->ops.dsat};
            case Fragment::WRAP_A: return {2 + subs[0]->ops.count, subs[0]->ops.min, subs[0]->ops.dsat};
            case Fragment::WRAP_D: return {3 + subs[0]->ops.count, subs[0]->ops.min, 0};
            case Fragment::WRAP_J: return {4 + subs[0]->ops.count, subs[0]->ops.min, 0};
            case Fragment::WRAP_V: return {subs[0]->ops.count + (subs[0]->GetType() << "x"_mst), subs[0]->ops.min, {}};
            case Fragment::THRESH: {
                uint32_t count = 0;
                ...
                for (const auto& sub : subs) {
                    ...
                    for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + sub->ops.dsat) | (sats[j - 1] + sub->ops.min));
                    next_sats.push_back(sats[sats.size() - 1] + sub->ops.min);
    ...
    internal::StackSize CalcStackSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::ANDOR: {
                ...
                return {
                    (x.min + SatInfo::If() + y.min) | (x.dsat + SatInfo::If() + z.min),
            ...
            case Fragment::AND_B: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {x.min + y.min + SatInfo::BinaryOp(), x.dsat + y.dsat + SatInfo::BinaryOp()};
            }
            case Fragment::OR_B: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {
                    ((x.min + y.dsat) | (x.dsat + y.min)) + SatInfo::BinaryOp(),
            ...
            case Fragment::OR_C: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {(x.min + SatInfo::If()) | (x.dsat + SatInfo::If() + y.min), {}};
            }
            case Fragment::OR_D: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {
                    (x.min + SatInfo::OP_IFDUP(true) + SatInfo::If()) | (x.dsat + SatInfo::OP_IFDUP(false) + SatInfo::If() + y.min),
            ...
            case Fragment::OR_I: {
                const auto& x{subs[0]->ss};
                const auto& y{subs[1]->ss};
                return {SatInfo::If() + (x.min | y.min), SatInfo::If() + (x.dsat | y.dsat)};
            }
            ...
            case Fragment::WRAP_C: return {
                subs[0]->ss.min + SatInfo::OP_CHECKSIG(),
                subs[0]->ss.dsat + SatInfo::OP_CHECKSIG()
            };
            case Fragment::WRAP_D: return {
                SatInfo::OP_DUP() + SatInfo::If() + subs[0]->ss.min,
                SatInfo::OP_DUP() + SatInfo::If()
            };
            case Fragment::WRAP_V: return {subs[0]->ss.min + SatInfo::OP_VERIFY(), {}};
            case Fragment::WRAP_J: return {
                SatInfo::OP_SIZE() + SatInfo::OP_0NOTEQUAL() + SatInfo::If() + subs[0]->ss.min,
            ...
            case Fragment::THRESH: {
                ...
                for (size_t i = 0; i < subs.size(); ++i) {
                    ...
                    for (size_t j = 1; j < sats.size(); ++j) {
                        next_sats.push_back(((sats[j] + subs[i]->ss.dsat) | (sats[j - 1] + subs[i]->ss.min)) + add);
                    }
                    // Finally construct next_sats[i+1].
                    next_sats.push_back(sats[sats.size() - 1] + subs[i]->ss.min + add);
                    // Switch over.
                    sats = std::move(next_sats);
                }
    ...
    internal::WitnessSize CalcWitnessSize() const {
        ...
        switch (fragment) {
            ...
            case Fragment::ANDOR: {
                const auto min{(subs[0]->ws.min + subs[1]->ws.min) | (subs[0]->ws.dsat + subs[2]->ws.min)};
                const auto dsat{subs[0]->ws.dsat + subs[2]->ws.dsat};
                return {min, dsat};
            }
            case Fragment::AND_V: return {subs[0]->ws.min + subs[1]->ws.min, {}};
            case Fragment::AND_B: return {subs[0]->ws.min + subs[1]->ws.min, subs[0]->ws.dsat + subs[1]->ws.dsat};
            case Fragment::OR_B: {
                const auto min{(subs[0]->ws.dsat + subs[1]->ws.min) | (subs[0]->ws.min + subs[1]->ws.dsat)};
                const auto dsat{subs[0]->ws.dsat + subs[1]->ws.dsat};
                return {min, dsat};
            }
            case Fragment::OR_C: return {subs[0]->ws.min | (subs[0]->ws.dsat + subs[1]->ws.min), {}};
            case Fragment::OR_D: return {subs[0]->ws.min | (subs[0]->ws.dsat + subs[1]->ws.min), subs[0]->ws.dsat + subs[1]->ws.dsat};
            case Fragment::OR_I: return {(subs[0]->ws.min + 1 + 1) | (subs[1]->ws.min + 1), (subs[0]->ws.dsat + 1 + 1) | (subs[1]->ws.dsat + 1)};
            ...
            case Fragment::WRAP_D: return {1 + 1 + subs[0]->ws.min, 1};
            case Fragment::WRAP_V: return {subs[0]->ws.min, {}};
            case Fragment::WRAP_J: return {subs[0]->ws.min, 1};
            case Fragment::THRESH: {
                auto sats = Vector(internal::MaxInt<uint32_t>(0));
                for (const auto& sub : subs) {
                    auto next_sats = Vector(sats[0] + sub->ws.dsat);
                    for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + sub->ws.dsat) | (sats[j - 1] + sub->ws.min));
                    next_sats.push_back(sats[sats.size() - 1] + sub->ws.min);
                    sats = std::move(next_sats);
                }
                assert(k <= sats.size());
    ...
    internal::InputResult ProduceInput(const Ctx& ctx) const {
            ...
            switch (node.fragment) {
                ...
                case Fragment::MULTI_A: {
                    ...
                    for (size_t i = 0; i < node.keys.size(); ++i) {
                        ...
                        auto min = InputStack(std::move(sig)).SetWithSig().SetAvailable(avail);
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + ZERO) | (std::move(sats[j - 1]) + min));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(min));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                ...
                case Fragment::MULTI: {
                    ...
                    for (size_t i = 0; i < node.keys.size(); ++i) {
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back(sats[j] | (std::move(sats[j - 1]) + min));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(min));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                ...
                case Fragment::THRESH: {
                    ...
                    for (size_t i = 0; i < subres.size(); ++i) {
                        ...
                        // so far. next_sats[j] is either sats[j] + res.nsat (reusing j earlier satisfactions) or sats[j-1] + res.min
                        ...
                        for (size_t j = 1; j < sats.size(); ++j) next_sats.push_back((sats[j] + res.nsat) | (std::move(sats[j - 1]) + res.min));
                        next_sats.push_back(std::move(sats[sats.size() - 1]) + std::move(res.min));
                        // Switch over.
                        sats = std::move(next_sats);
                    }
                    // At this point, sats[k].min is the best satisfaction for the overall thresh() node. The best dissatisfaction
                ...
                case Fragment::AND_V: {
                    ...
                    return {(y.nsat + x.min).SetNonCanon(), y.min + x.min};
                }
                case Fragment::AND_B: {
                    auto& x = subres[0], &y = subres[1];
                    ...
                    return {(y.nsat + x.nsat) | (y.min + x.nsat).SetMalleable().SetNonCanon() | (y.nsat + x.min).SetMalleable().SetNonCanon(), y.min + x.min};
                }
                case Fragment::OR_B: {
                    auto& x = subres[0], &z = subres[1];
                    // The (min(Z) min(X)) solution is overcomplete (attacker can change either into dsat).
                    return {z.nsat + x.nsat, (z.nsat + x.min) | (z.min + x.nsat) | (z.min + x.min).SetMalleable().SetNonCanon()};
                }
                case Fragment::OR_C: {
                    auto& x = subres[0], &z = subres[1];
                    return {INVALID, std::move(x.min) | (z.min + x.nsat)};
                }
                case Fragment::OR_D: {
                    auto& x = subres[0], &z = subres[1];
                    return {z.nsat + x.nsat, std::move(x.min) | (z.min + x.nsat)};
                }
                case Fragment::OR_I: {
                    auto& x = subres[0], &z = subres[1];
                    return {(x.nsat + ONE) | (z.nsat + ZERO), (x.min + ONE) | (z.min + ZERO)};
                }
                case Fragment::ANDOR: {
                    auto& x = subres[0], &y = subres[1], &z = subres[2];
                    return {(y.nsat + x.min).SetNonCanon() | (z.nsat + x.nsat), (y.min + x.min) | (z.min + x.nsat)};
                }
                case Fragment::WRAP_A:
                case Fragment::WRAP_S:
                case Fragment::WRAP_C:
                ...
                case Fragment::WRAP_J: {
                    ...
                    return {InputStack(ZERO).SetMalleable(x.nsat.available != Availability::NO && !x.nsat.has_sig), std::move(x.min)};
                }
                case Fragment::WRAP_V: {
                    auto &x = subres[0];
                    return {INVALID, std::move(x.min)};
                }
        ...
        auto tester = [&helper](const Node& node, Span<InputResult> subres) -> InputResult {
            ...
            if (node.GetType() << "z"_mst && ret.min.available != Availability::NO) assert(ret.min.stack.size() == 0);
            ...
            if (node.GetType() << "o"_mst && ret.min.available != Availability::NO) assert(ret.min.stack.size() == 1);
            ...
            if (node.GetType() << "n"_mst && ret.min.available != Availability::NO) assert(ret.min.stack.size() >= 1);
            if (node.GetType() << "n"_mst && ret.nsat.available != Availability::NO) assert(ret.nsat.stack.size() >= 1);
            if (node.GetType() << "n"_mst && ret.min.available != Availability::NO) assert(!ret.min.stack.back().empty());
            ...
            if (node.GetType() << "s"_mst && ret.min.available != Availability::NO) assert(ret.min.has_sig);
            ...
            if (node.GetType() << "m"_mst && ret.min.available != Availability::NO) assert(!ret.min.malleable);

            // If a non-malleable satisfaction exists, it must be canonical.
            if (ret.min.available != Availability::NO && !ret.min.malleable) assert(!ret.min.non_canon);
...
public:
    ...
    std::optional<uint32_t> GetOps() const {
        if (!ops.min.valid) return {};
        return ops.count + ops.min.value;
    }
    ...
    std::optional<uint32_t> GetStackSize() const {
        if (!ss.min.valid) return {};
        return ss.min.netdiff + static_cast<int32_t>(IsBKW());
    }

    //! Return the maximum size of the stack during execution of this script.
    std::optional<uint32_t> GetExecStackSize() const {
        if (!ss.min.valid) return {};
        return ss.min.exec + static_cast<int32_t>(IsBKW());
    }
    ...
    Availability Satisfy(const Ctx& ctx, std::vector<std::vector<unsigned char>>& stack, bool nonmalleable = true) const {
        ...
        if (nonmalleable && (ret.min.malleable || !ret.min.has_sig)) return Availability::NO;
        stack = std::move(ret.min.stack);
        return ret.min.available;
    }
...
//! Parse a miniscript from a tortoisecoin script
...
#endif // TORTOISECOIN_SCRIPT_MINISCRIPT_H
