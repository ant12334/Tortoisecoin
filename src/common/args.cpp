...
const char * const TORTOISECOIN_CONF_FILENAME = "tortoisecoin.conf";
const char * const TORTOISECOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //tortoisecoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", TORTOISECOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Tortoisecoin
    //   new: C:\Users\Username\AppData\Local\Tortoisecoin
    // macOS: ~/Library/Application Support/Tortoisecoin
    // Unix-like: ~/.tortoisecoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Tortoisecoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Tortoisecoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Tortoisecoin";
#else
    // Unix-like
    return pathRet / ".tortoisecoin";
#endif
#endif
}
