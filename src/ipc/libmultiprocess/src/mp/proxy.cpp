...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking tortoisecoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before tortoisecoin
    // code in the destructors are run. This way if the tortoisecoin code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In tortoisecoin core, running these callbacks in the right order is
