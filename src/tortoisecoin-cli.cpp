...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", TORTOISECOIN_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-generate",
                   strprintf("Generate blocks, equivalent to RPC getnewaddress followed by RPC generatetoaddress. Optional positional integer "
                             ...
                             "RPC generatetoaddress nblocks and maxtries arguments. Example: tortoisecoin-cli -generate 4 1000",
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to tortoisecoind). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The tortoisecoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: tortoisecoin-cli [options] <command> [params]\n"
                "or:    tortoisecoin-cli [options] -named <command> [name=value]...\n"
                "or:    tortoisecoin-cli [options] help\n"
                "or:    tortoisecoin-cli [options] help <command>\n"
                "\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires tortoisecoind server to be running v22.0 and up");
        }
...
public:
    ...
    UniValue PrepareRequest(const std::string& method, const std::vector<std::string>& args) override
    {
        if (!args.empty()) {
            ...
            if (ParseUInt8(args.at(0), &n)) {
                ...
                throw std::runtime_error(strprintf("invalid -netinfo level argument: %s\nFor more information, run: tortoisecoin-cli -netinfo help", args.at(0)));
            }
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    m_outbound_only_selected = true;
                } else if (n) {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: tortoisecoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: tortoisecoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    UniValue ProcessReply(const UniValue& batch_in) override
    {
        ...
        if (networkinfo["version"].getInt<int>() < 209900) {
            throw std::runtime_error("-netinfo requires tortoisecoind server to be running v0.21.0 and up");
        }
    ...
    const std::string m_help_doc{
        ...
        "  version  Peer version and subversion concatenated, e.g. \"70016/mini:21.0.0/\"\n\n"
        ...
        "> tortoisecoin-cli -netinfo\n\n"
        "The same, preceded by a peers listing without address and version columns\n"
        "> tortoisecoin-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> tortoisecoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "Full dashboard, but with outbound peers only\n"
        + strprintf("> tortoisecoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title tortoisecoin-cli -netinfo %d\n\n", NETINFO_MAX_LEVEL) +
        "See this help\n"
        "> tortoisecoin-cli -netinfo help\n"};
};
...
static UniValue CallRPC(BaseRequestHandler* rh, const std::string& strMethod, const std::vector<std::string>& args, const std::optional<std::string>& rpcwallet = {})
{
    ...
    if (response.status == 0) {
        ...
        throw CConnectionFailed(strprintf("Could not connect to the server %s:%d%s\n\n"
                    "Make sure the tortoisecoind server is running and that you are connecting to the correct RPC port.\n"
                    "Use \"tortoisecoin-cli -help\" for more info.",
...
static void ParseError(const UniValue& error, std::string& strPrint, int& nRet)
{
    if (error.isObject()) {
        ...
        if (err_code.isNum() && err_code.getInt<int>() == RPC_WALLET_NOT_SPECIFIED) {
            ...
            strPrint += " (run \"tortoisecoin-cli -h\" for help or \"tortoisecoin-cli listwallets\" to see which wallets are currently loaded).";
        }
    } else {
