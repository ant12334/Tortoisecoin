...
Tortoisecoin Core has an internal benchmarking framework, with benchmarks
...
For benchmarking, you only need to compile `bench_tortoisecoin`.  The bench runner
warns if you configure with `-DCMAKE_BUILD_TYPE=Debug`, but consider if building without
it will impact the benchmark(s) you are interested in by unlatching log printers
and lock analysis.

    cmake -B build -DBUILD_BENCH=ON
    cmake --build build -t bench_tortoisecoin

After compiling tortoisecoin-core, the benchmarks can be run with:

    build/bin/bench_tortoisecoin
...
---------------------
    ...
    build/bin/bench_tortoisecoin -h
...
To monitor Tortoisecoin Core performance more in depth (like reindex or IBD): https://github.com/tortoisecoin-dev-tools/benchcoin
