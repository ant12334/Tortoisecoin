...
To build Tortoisecoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
...
to: /path/to/tortoisecoin/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/tortoisecoin/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Tortoisecoin Core can
