...
This guide describes how to build tortoisecoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
### 3. Clone Tortoisecoin Repository
...
The Tortoisecoin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the Tortoisecoin Core project provides presets, which are used in this guide.
...
To improve the build process performance, one might add the Tortoisecoin repository directory to the Microsoft Defender Antivirus exclusions.
