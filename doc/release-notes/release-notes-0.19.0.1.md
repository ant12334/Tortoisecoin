Tortoisecoin Core version 0.19.0.1 is now available from:
  ...
  <https://bitcoin.org/bin/tortoisecoin-core-0.19.0.1/>
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  TTC), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in TTC per 1,000
...
- #16521 Use the default maxfeerate value as TTC/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as TTC/kB (jonatack)
