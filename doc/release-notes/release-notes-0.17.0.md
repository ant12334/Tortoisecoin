...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `tortoisecoin.conf` file, not inside included files or from
  ...
  as tortoisecoin.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the tortoisecoin.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=tortoisecoin
    test.uacomment=tortoisecoin-testnet
...
be used in V0.17 by starting tortoisecoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Tortoisecoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/tortoisecoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Tortoisecoin transactions that are not fully signed
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When tortoisecoin is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If tortoisecoin is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart tortoisecoind with
...
- When running tortoisecoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If tortoisecoind
...
- #11962 `0a01843` add seed.tortoisecoin.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI tortoisecoin:// (krab)
...
- #13264 `2a7c53b` mini unit (GreatSock)
...
- #13528 `b877c39` Move TortoisecoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libtortoisecoin-util to libtortoisecoin-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for tortoisecoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling tortoisecoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use TortoisecoinTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to tortoisecoin.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use TortoisecoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use TortoisecoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use TortoisecoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for TortoisecoinTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_tortoisecoin` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.tortoisecoin is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_tortoisecoin` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in tortoisecoind systemd service file (Flowdalic)
- #12652 `55f490a` tortoisecoin-cli: Provide a better error message when tortoisecoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to tortoisecoind in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for tortoisecoin address in translations (kaplanmaxe)
...
- #13603 `453ae5e` tortoisecoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix tortoisecoin-cli --version (Empact)
...
- #12877 `2b54155` Use tortoisecoind in Tor documentation (knoxcard)
...
- #13346 `60f0358` update tortoisecoin-dot-org links in release-process.md (fanquake)
...
- #13905 `73a09b4` fixed tortoisecoin-cli -help output for help2man (hebasto)
