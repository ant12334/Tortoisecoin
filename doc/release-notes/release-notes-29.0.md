Tortoisecoin Core version 29.0 is now available from:
  ...
  <https://bitcoin.org/bin/tortoisecoin-core-29.0/>
...
Tortoisecoin Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Tortoisecoin
...
- As a safety check, Tortoisecoin core will **fail to start** when `-blockreservedweight` init parameter value is lower than `2000` weight units. Tortoisecoin Core will also **fail to start** if the `-blockmaxweight` or `-blockreservedweight` init parameter exceeds consensus limit of `4,000,000 WU`.
...
3. CMake variables may be used to configure the build system. **Some defaults have changed.** For example, you will now need to add `-DWITH_ZMQ=ON` to build with zmq and `-DBUILD_GUI=ON` to build `tortoisecoin-qt`. See [Autotools to CMake Options Mapping](https://github.com/tortoisecoin-core/tortoisecoin-devwiki/wiki/Autotools-to-CMake-Options-Mapping) for details.
4. For single-configuration generators, the default build configuration (`CMAKE_BUILD_TYPE`) is "RelWithDebInfo". However, for the "Release" configuration, CMake defaults to the compiler optimization flag `-O3`, which has not been extensively tested with Tortoisecoin Core. Therefore, the build system replaces it with `-O2`.
...
- A new tool [`utxo_to_sqlite.py`](https://github.com/tortoisecoin/tortoisecoin/blob/v29.0/contrib/utxo-tools/utxo_to_sqlite.py)
