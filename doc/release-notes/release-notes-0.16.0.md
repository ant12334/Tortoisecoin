Tortoisecoin Core version 0.16.0 is now available from:

  <https://tortoisecoincore.org/bin/tortoisecoin-core-0.16.0/>
...
Tortoisecoin Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
tortoisecoin data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Tortoisecoin Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µTTC" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- tortoisecoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- tortoisecoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest tortoisecoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.tortoisecoin.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for tortoisecoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to tortoisecoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #12035 `eeb6d52` Change µTTC to bits (jb55)
...
- #11621 `1f7695b` [build] Add temp_tortoisecoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` tortoisecoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.tortoisecoin doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for tortoisecoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in tortoisecoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop tortoisecoind (OmeGak)
...
- #10529 `331352f` Improve tortoisecoind systemd service file (Flowdalic)
