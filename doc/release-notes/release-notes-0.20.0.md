...
Tortoisecoin Core version 0.20.0 is now available from:

  <https://tortoisecoincore.org/bin/tortoisecoin-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Tortoisecoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Tortoisecoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Tortoisecoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Tortoisecoin Core. Tortoisecoin Core logs debug messages
...
- OpenSSL is no longer used by Tortoisecoin Core.  (#17265)

- BIP70 support has been fully removed from Tortoisecoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/tortoisecoin/tortoisecoin/issues/16599),
  [PR #16702](https://github.com/tortoisecoin/tortoisecoin/pull/16702), and the
  `tortoisecoind help` for more information.  This option is experimental and
...
- All custom settings configured when Tortoisecoin Core starts are now
...
- The "Start Tortoisecoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://tortoisecoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Tortoisecoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Tortoisecoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/tortoisecoin-core/HWI) for
  signing.  Future versions of Tortoisecoin Core should support a GUI option
...
- Tortoisecoin Core's automatically-generated source code documentation is
  now available at https://doxygen.tortoisecoincore.org.  (#17596)
...
- The `tortoisecoin-cli` utility used with the `-getinfo` parameter now
...
- The `tortoisecoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `tortoisecoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_tortoisecoin-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_tortoisecoin` binary (hebasto)
...
- #18702 Fix ASLR for tortoisecoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate tortoisecoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest tortoisecoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_tortoisecoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (tortoisecoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone tortoisecoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Tortoisecoin Core files (hebasto)
...
- #17119 Fix broken tortoisecoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from tortoisecoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.tortoisecoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to tortoisecoin-wallet (hel-o)
...
- #18341 Replace remaining literal TTC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On tortoisecoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from tortoisecoin-cli -getinfo (malevolent)
