...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  to sat/vB and the help documentation in TTC/kB is updated to TTC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from TTC/kvB to sat/vB and the
  ...
  difference between TTC/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in TTC/kvB rather than sat/vB should raise an
