...
Tortoisecoin Core version 0.21.0 is now available from:
  ...
  <https://bitcoincore.org/bin/tortoisecoin-core-0.21.0/>
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (min/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in TTC per 1,000
  vbytes (TTC/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from TTC/kB
  to min/vB and the help documentation in TTC/kB is updated to TTC/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from TTC/kvB to min/vB and the
  ...
  difference between TTC/kvB and min/vB units means that a transaction with a
  fee rate mistakenly calculated in TTC/kvB rather than min/vB should raise an
  error due to the fee rate being set too low. In the worst case, the
  transaction may send at 1 min/vB, but as Replace-by-Fee (BIP125 RBF) is active
...
- #20305 Introduce `fee_rate` min/vB param/option (jonatack)
