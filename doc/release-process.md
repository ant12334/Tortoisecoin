...
* Update tortoisecoin.conf and commit changes if they exist, see [gen-tortoisecoin-conf.sh](/contrib/devtools/README.md#gen-tortoisecoin-confsh).
...
* On the new release branch in [`CMakeLists.txt`](../CMakeLists.txt)(see [this commit](https://github.com/tortoisecoin/tortoisecoin/commit/742f7dd)):
...
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `m_assumeutxo_data` array should be appended to with the values returned by calling `tortoisecoin-cli -rpcclienttimeout=0 -named dumptxoutset utxo.dat rollback=<height or hash>`
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf tortoisecoin-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf tortoisecoin-${VERSION}-win64-codesigning.tar.gz
...
- Upload to the bitcoin.org server:
...
- Update the bitcoin.org website

  - blog post

  - maintained versions [table](https://github.com/tortoisecoin-core/bitcoin.org/commits/master/_includes/posts/maintenance-table.md)

  - RPC documentation update

      - See https://github.com/tortoisecoin-core/bitcoin.org/blob/master/contrib/doc-gen/
...
- Announce the release:
  ...
  - Tortoisecoin Core announcements list https://bitcoin.org/en/list/announcements/join/
